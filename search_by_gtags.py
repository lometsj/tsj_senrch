...
class CodeAnalyzer:
    ...
    def get_symbol_info(self, symbol: str) -> Dict:
        ...
        try:
            ...
            file_path, line_num = parts[2], parts[1]
            ...
            if int(line_num) <= len(content):
                ...
                if "#define" in definition_line:
                    ...
                    return {"type": "function", "definition": tool.find_function_in_c_file(file_path, symbol)}
                else:
    ...
    def get_call_by_info(self, symbol: str) -> List[Dict]:
        ...
        try:
            ...
            for line in lines:
                ...
                file_path, line_num, call_line = parts[2], parts[1], parts[3]
                # context = tool.find_function_in_c_file(os.path.join(self.code_dir, file_path), caller_func)
                context = tool.get_context_of_function(os.path.join(self.code_dir, file_path), int(line_num))
                caller_func = tool.get_callerfunc_by_file_and_line(file_path, line_num, self.code_dir)                       
                calls.append({
                    "file": file_path,
                    "line": line_num,
                    "caller": caller_func,
    ...
    def find_call_paths(
        ...
        for log_func in log_functions:
            ...
            for call in direct_calls:
                # 初始化路径：包含日志函数和直接调用者的信息
                initial_path = [
                    {
                        "function": log_func,
                        **call  # 合并调用者信息（如行号、文件等）
                    }
                ]
                current_func = call["caller"]
                current_depth = 1  # 初始深度为1（已包含直接调用者）
                
                # 收集所有可能的路径分支
                paths = []
                
                def _trace_path(current_path, current_func, depth):
                    if depth >= max_depth or current_func == "main":
                        paths.append(current_path.copy())  # 保存路径
                        return
                    # 获取当前函数的所有调用者
                    callers = self.get_call_by_info(current_func)
                    if not callers:
                        paths.append(current_path.copy())  # 无上层调用时也保存路径
                        return
                    # 遍历所有调用者，生成分支路径
                ...
                _trace_path(initial_path, current_func, current_depth)
                result[log_func].extend(paths)
...
class LLMAnalyzer:
    ...
    def prepare_context(self, log_func: str, call: List[Dict]) -> str:
        """准备用于查询LLM的上下文"""
        context = f"我将提供一个日志打印函数{log_func}的调用点, 这个函数的参数将会在日志中输出，请判断它是否打印了敏感信息（如密码、密钥、令牌等）。对于每一个参数，你应该详细分析该参数的来源来判断，比如如果打印某个变量，使用get_symbol获取该变量结构体，如果某个变量的值不确定为常量，使用call_by获取调用函数信息向上追踪数据流，已经为你分析了整个代码库，所以你可以请求更多信息。\n\n"
        context += "【输出结果要求】\n"
        context += "请在回答中包含带tsj的标签，以下标签三选一:\n"
        context += "- 如判断有敏感信息（只判断当前调用路径的{}打印函数）: [tsj_have] 并提供 {{\"sensitive_type\": \"敏感类型\", \"context\": \"代码上下文\"}}\n".format(log_func)
        context += "- 如判断无敏感信息: [tsj_nothave]\n"
        # context += "- 分析完成后请包含: [tsj_end]\n"
        context += "- 如果不能判断，需要获取信息进一步分析，请包含[tsj_next]，并包含get_symbol或者call_by请求获取更多代码信息,详细格式如下："
        context += "1. 如果需要知道某个函数，宏或者变量的定义，使用get_symbol获取符号信息: {\"command\": \"get_symbol\", \"sym_name\": \"符号名称\"}\n"
        context += "2. 如果需要进一步分析数据流，使用call_by获取调用信息: {\"command\": \"call_by\", \"sym_name\": \"符号名称\"}\n\n"

        # 添加call
        context += f"文件: {call['file']}, 行号: {call['line']}\n"
        context += f"函数: {call['caller']}\n"
        context += f"代码上下文: {call['context']}\n"
        context += f"调用行： {call['call_line']}\n"
    ...
    def analyze_log_function(self, log_func: str, call_list: List[List[Dict]]) -> List[Dict]:
        """分析单个日志函数的所有调用路径"""
        results = []
        
        for path_index, call in enumerate(call_list):
            logger.info(f"分析 {log_func} 的调用路径 {path_index+1}/{len(call_list)}")
            
            # 初始化对话
            messages = [
                {"role": "system", "content": "你是一个代码安全分析专家，专注于识别代码中的敏感信息泄露。"},
                {"role": "user", "content": self.prepare_context(log_func, call)}
            ]
            ...
            while not conversation_complete and turn < max_turns:
                ...
                if not conversation_complete:
                    ...
                    if requests:
                        ...
                        response_message = "！！！！！！！！！！！！！！！需要进一步请求以下是请求的附加信息:\n\n" + json.dumps(responses, ensure_ascii=False, indent=2)
    ...
    def generate_html_report(self, results: Dict, output_file: str = None) -> str:
        ...
        for log_func, func_results in results.items():
            ...
            for idx, result in enumerate(func_results):
                ...
                detailed_results += f'  caller函数: {html.escape(call["caller"])}\n'
                detailed_results += f'  调用行：{html.escape(call["call_line"])}\n'
                ...
                for msg in result["conversation"]:
                    ...
                    detailed_results += html.escape(msg["content"]).replace("[tsj_have]", "<span class=\'sensitive-info\'>[敏感信息]</span>").replace("[tsj_nothave]", "<span>[无敏感信息]</span>").replace("[tsj_next]", "<span>[需要进一步分析]</span>").replace("\n", "<br>")
                    detailed_results += '</div>\n'
...
def main():
    ...
    parser.add_argument('--base-url', default='', help="base url 设置")
    parser.add_argument('--model', default='deepseek-reasoner', help="模型选择")
    ...
    for log_function in log_functions:
        call_list[log_function] = code_analyzer.get_call_by_info(log_function)

    # 结构化输出调用路径
    for log_func, paths in call_list.items():
        print(f"\n日志函数: {log_func}")
        print("-" * 50)
        for i, call in enumerate(paths, 1):
            print(f"\n调用点 {i}:")
    ...
    # 初始化LLM分析器
    llm_analyzer = LLMAnalyzer(code_analyzer, args.api_key, args.base_url, args.model)
    
    # 分析每个日志函数
    results = {}
    for log_func, call in call_list.items():
        logger.info(f"使用LLM分析日志函数 {log_func} 的 {len(paths)} 条调用路径")
        results[log_func] = llm_analyzer.analyze_log_function(log_func, call)
