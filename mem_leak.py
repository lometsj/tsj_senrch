class mem_leak_problem:
    ...
    def prepare_context(task):
        ...
        prompt['init_user'] = f"""【任务】
我将提供一个内存申请函数{malloc_func}的调用点代码, 这个函数会动态申请内存，并由{free_func}释放，请判断该内存申请后有没有释放，如果没有释放，就有内存泄露问题。
【注意】
你必须只需要分析内存泄露问题，其他的不要管。
你应该只分析单函数场景{malloc_func}申请出来的内存是否有内存泄露问题，比如异常分支没有释放，局部变量持有的内存在函数return前没有释放。
